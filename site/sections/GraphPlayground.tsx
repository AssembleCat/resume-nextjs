...
export function GraphPlayground({
  ...
  return (
    <section id="map" className="w-full py-16">
      <div className="mx-auto mb-8 flex max-w-6xl flex-col gap-4 px-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-400">
            {pipeline?.subtitle ||
              '회사·프로젝트·수상의 연결입니다. 한눈은 위 타임라인, 여기는 관계를 봅니다.'}
          </p>
        </div>
        ...
        <LayoutGroup>
          {DIAGRAM_TABS.map((tab) => (
            <button
              ...
              className={`relative px-4 py-2 font-mono text-[11px] uppercase tracking-[0.14em] ${
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-2 text-xl font-semibold leading-snug">{detail.title}</h3>
              </div>
              <button
                type="button"
                onClick={() => onSelect(undefined)}
                className="border border-white/15 px-3 py-1.5 text-xs text-zinc-400 transition hover:border-white/40 hover:text-white"
              >
                닫기
              </button>
            </div>
